...
\begin{remark}
Since the derivative of a function tells us where the function is increasing/decreasing, we will use the second derivative to determine where the first derivative is increasing/decreasing.
\end{remark}

\vfill

\begin{remark}
Recall that the graph of $y=f(x)$ has a tangent line at $x=c$ if 
\begin{enumerate}
\item $f'(c)$ exists,
\item $\DS\lim_{x\to c}f'(x) = \infty$, or
\item $\DS\lim_{x\to c}f'(x) = -\infty$.
\end{enumerate}
The last two possibilities correspond to vertical tangent lines.
\end{remark}
...
\begin{example}
Observe that the second derivative test fails for the function $f(x)=x^3$.
\end{example}
