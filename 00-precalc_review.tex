...
\begin{remark}
A function is mathematical model for expressing a relationship between two variables (or measurements), say $x$ and $y$, 
where each valid choice of $x$ (i.e., each $x$ in the domain) gives rise to exactly one corresponding $y$.
For example, $x$ might represent a distance relative to some fixed point,
and $y = f(x)$ might represent the potential energy stored in a certain object when placed at distance $x$.
\end{remark}
...
\begin{enumerate}
\item $f(x)=\DS\frac{x^2+1}{x}$ and $g(x)=\DS\frac{x+1}{x-1}$.
\vfill
