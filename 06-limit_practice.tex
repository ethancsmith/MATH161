...
\begin{example}
Suppose that $n=4$ moles of an ideal gas are kept in a $V=100$ liter container.
What temperature $T$ is required to maintain an absolute pressure of $p = 1$ atmosphere?
Suppose that we allow for an $\epsilon$ tolerance in pressure.
What $\delta$-tolerance is allowed in temperature?
Hint: The ideal gas law states that $pV = nRT$, where $R = 0.08206$ \si{\frac{L\cdot atm}{mol\cdot K}} is the ideal gas constant.
You may give your results in terms of $R$ rather than its numerical value.
\end{example}
...
Let 
\begin{equation*}
f(x) = \begin{cases} 5x-3 & \text{if } x\ne 1,\\ 3 & \text{if } x=1.\end{cases}
\end{equation*}
Prove that $\DS\lim_{x\to 1}f(x) = 2$ by appealing to the \underline{definition} of limit.
\end{example}
