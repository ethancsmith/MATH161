...
\lectureheader{161}{Calculus I}{Trigonometry review}{\textit{Thomas' Calculus} \thesection}
...
\begin{definition}
The \textbf{radian measure} of a counterclockwise-oriented, central angle within a circle of radius $r$ is $$\theta = s/r,$$ where $s$ is the length of the arc subtended by the angle (measured in the same units as $r$).
\end{definition}
...
\begin{remark}\,
\begin{itemize}
\item As the ratio of 2 lengths (measured according to the same unit of length), radians are \textbf{dimensionless} and \textbf{unitless}.
...
\item In calculus, we always default to radian measure (e.g., $\theta=60$ means 60 radians, not 60\textdegree).
\item Degrees are dimensionless but not unitless: $1^\circ=\frac{\pi}{180}$.
\end{itemize}
\end{remark}
...
\begin{remark}
You absolutely need to learn to use reference triangles.
While some problems can be solved by ``memorizing the unit circle," reference triangles are a more versatile tool that can be applied in many situations where a the so-called memorized circle fails to get the job done.
\end{remark}
...
\begin{example}
Evaluate all 6 trigonometric functions at the following values of $\theta$.
\begin{enumerate}
\item $\theta = 2\pi/3$
\vfill
\item $\theta=-5\pi/6$
